...
const AuthModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        localStorage.setItem('dharma_token', data.token);
  ...
  const handleGoogleLogin = () => {
    setError('Google login will be available soon! Please use email/password for now.');
  };

  const handleFacebookLogin = () => {
    setError('Facebook login will be available soon! Please use email/password for now.');
  };
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md transform transition-all">
        ...
        <div className="bg-gradient-to-r from-orange-500 to-pink-500 text-white p-6 rounded-t-2xl">
          <div className="flex justify-between items-center">
            <div>
              <h2 className="text-2xl font-bold">🕉️ DharmaVerse</h2>
              <p className="text-orange-100">
                {isLogin ? 'Welcome back, seeker!' : 'Begin your spiritual journey'}
              </p>
            </div>
            <button 
              onClick={onClose}
        ...
        <div className="p-6">
          ...
          <div className="space-y-3 mb-6">
            ...
            <button 
              ...
              className="w-full flex items-center justify-center gap-3 bg-gray-800 hover:bg-gray-900 text-white py-3 px-4 rounded-lg font-medium transition-colors"
            >
              <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
